...
def generate_product_sales_history(product_id, period='monthly', branch_id=None):
    """Generar historial de ventas de un producto específico"""
    conn = db_service.get_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    ...
    query = f"""
        ...
        FROM sale_items si
        JOIN sales s ON si.sale_id = s.id
        WHERE si.product_id = %s AND s.sale_date >= %s
    """
    ...
    query += f" GROUP BY {group_by}, {period_format} ORDER BY {group_by}"
    ...
    for h in history:
        sales_history.append({
            'period': h['period'],
            'period_date': h['period_date'].strftime('%Y-%m-%d') if h['period_date'] else None,
            'quantity': h['quantity'],
            'totalAmount': float(h['total_amount']),
    ...
    branch_params = [product_id, date_from]
    
    cur.execute(branch_query, branch_params)
    branch_sales = cur.fetchall()
    ...
    result = {
        ...
        'branchSales': branch_data,
        'totalQuantity': totals['total_quantity'] or 0,
