...
def validate_branch(branch_data):
    ...
    if 'phone' in branch_data and (not isinstance(branch_data['phone'], str) or not re.match(r'^\+?[\d\s\-\(\)]{7,20}$', branch_data['phone'])):
        return False
    
    if 'manager' in branch_data and (not isinstance(branch_data['manager'], str) or len(branch_data['manager']) < 3):
        return False
...
def validate_product(product_data):
    ...
    # Validar campos opcionales si están presentes
    if 'description' in product_data and not isinstance(product_data['description'], str):
        return False
    
    if 'category' in product_data and not isinstance(product_data['category'], str):
        return False
...
def validate_user(user_data):
    ...
    # Validar branch_id si está presente
    if 'branch_id' in user_data and not isinstance(user_data['branch_id'], int):
        return False
